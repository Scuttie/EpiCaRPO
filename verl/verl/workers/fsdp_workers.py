...
try:
    ...
    pass # nest_asyncio가 설치되어 있지 않을 경우를 대비
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        with open_dict(self.config.rollout):
            # RayTrainer가 사용하는 'enable' 플래그를 백업해두고 잠시 삭제합니다.
            has_agent = 'agent' in self.config.rollout
            enable_val = None
            if has_agent and 'enable' in self.config.rollout.agent:
                enable_val = self.config.rollout.agent.pop('enable')
        
        # 인스턴스화 수행 (이제 'enable'이 없으므로 TypeError가 나지 않습니다)
        rollout_config: RolloutConfig = omega_conf_to_dataclass(self.config.rollout)
        
        # 트레이너의 다른 로직을 위해 다시 복구해둡니다.
        if enable_val is not None:
            with open_dict(self.config.rollout):
                self.config.rollout.agent.enable = enable_val
