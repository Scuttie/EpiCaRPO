...
class RolloutReplica(ABC):
    ...
    def __init__(
        ...
        # --- 수정 시작: AgentLoopConfig의 'enable' 인자 충돌 방지 ---
        from omegaconf import open_dict, DictConfig
        if isinstance(config, DictConfig) and 'agent' in config:
            with open_dict(config):
                # 'enable' 키가 있으면 제거 (이미 트레이너에서 True임을 확인하고 이 단계에 옴)
                config.agent.pop('enable', None)
